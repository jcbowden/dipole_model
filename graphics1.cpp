...
static void   RenderWindow(void) ;						/* for rendering callback function */

static void   ChangeViewport(GLsizei w, GLsizei h) ;		/* for resize callback function */
static void   WindowVisible(int vis) ;					/* glutVisibilityFunc() */
static void   setEyePos() ;								/* glutIdleFunc() */
static void   keyFunction(unsigned char key, int x, int y) ;  
static void   mouseButtonFunction(int button, int state, int xPos, int yPos) ;
static void   mouseMoveFunction(int xPos, int yPos) ;
...
//double __cdecl  dToR(double deg) ;
float   getMinVal(int AmideIorII, float graphvals[2][36])  ;
void   CalculateMultiElectricVectAngle() ;
...
collagenIRInteraction  drawCollagenBonds(float spacing, float bondLength, struct spherCoords direction, int numBonds, float  IREvectAngle ) 
{
	...
	if (distTemp != 0.0) 
	{
	   ...
	   glEnd() ;
	   transformPoint(modelview,p2,p1) ;  // p1 is the transformed p2 xyz value
	   if (fp != NULL) 
	      fprintf(fp, "\n %5.3f %5.3f %5.3f",   p1[0],  p1[1],  p1[2]) ;
...
//	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT) ; // this does the clearing to the above clear color
	...
	// create the filename
	strcpy( str1, "xyz_data_ " );
	sprintf( str2,  "gaus_stdev%2.0f", gaussStdev );
	if ( fullOrHalf == 1 )
		   strcpy( str1, "Symetric_" ); 
	else if ( fullOrHalf == 2 ) 
		   strcpy( str1, "HalfSymetric_" );   
	else if ( fullOrHalf == 3 ) 
		   strcpy( str1, "NonSymetric_" );  
	else if ( fullOrHalf == 4 ) 
		   strcpy( str1, "SymetricRandom_" ); 
	strcat( str1, str2 );
	strcat( str1, ".txt" );
